...
Given a list of numbers and a specific number, let's invent a process to count how many numbers in the list are smaller than that specific number.  For example, if the list is `[10, 45, -2, 109]` and number is `13` then there are `2` numbers smaller than `13` in the list.
...
Your friend then asks you to count how many index cards contain a number smaller than the number on the sticky note and hand them a sticky note with the answer written on it.
